...
* __单纯享元模式__:具体的享元（ConcreteFlyweight）角色都可用于共享。
* __复合享元模式__:复合享元角色所代表的对象是不可以共享的,但是一个复合享元对象可以分解成为多个本身是单纯享元对象的组合。复合享元角色又称作不可共享的享元对象。

### 模式结构

* 享元模式: __单纯享元__

![享元模式UML结构图-单纯享元](https://github.com/Byronlee/byronlee.github.io/blob/master/images/jiandan_xiangyuan_uml.png?raw=true)

* 享元模式: __符合享元__

![享元模式UML结构图-复合享元](https://github.com/Byronlee/byronlee.github.io/blob/master/images/fuhe_xiangyuan_uml.png?raw=true)

整体来说,享元模式存在如下几个角色：

* Flyweight: 抽象享元类。是所有的具体享元角色的超类，为这些类规定出需要实现的公共接口。那些需要外部状态的操作可以通过以参数形式传入。
* ConcreteFlyweight: 具体享元类。指定内部状态，为内部状态提供存储空间。享元对象的内部状态必须与对象所处的周围环境无关
* UnsharedConcreteFlyweight: 非共享具体享元类。指出那些不需要共享的Flyweight子类。复合享元角色所代表的对象是不可以共享的，但是一个复合享元对象可以分解成为多个本身是单纯享元对象的组合。复合享元角色又称作不可共享的享元对象。
...
* 他们主要区别就在元复合享元模式比单纯享元多了一个UnsharedConcreteFlyweight,而这个类又是不可共享的,但是一个复合享元对象可以分解成为多个本身是单纯享元对象的组合


__享元模式的核心在于享元工厂类，享元工厂类的作用在于提供一个用于存储享元对象的享元池，用户需要对象时，首先从享元池中获取，如果享元池中不存在，则创建一个新的享元对象返回给用户，并在享元池中保存该新增对象__
...
### 模式实现
  ...
  这里是一个单纯享元模式,复合享元模式使用了组合模式,在组合模式中我们再做探讨
...
Ruby中要实现享元模式，跟Java思路差不多，可以用一样的类图，代码逻辑没有大的变化，__可以用ruby代码将上面的例子重写一遍.__ 

在ruby中值得一提可以改进或者不一样的地方目前想到的地方主要是： 再保持一个对象中的变与不变的地方，可以用一起ruby特有的方式元编程来处理，将变的部分动态的放入对象,
想到的有一下几点：

* 可以将动态的行为用 Module#define_method 来定义：`myClass.send(:you_behave,arg)`
* 使用闭包，将变的部分以代码块的形式传入对象，再使用：block_given?,yeild,call等配合使用
* 扁平作用域
  * 例1：` myClass.instance_eval{ # you changed something }`
  * 例2：` myClass.class_eval{ # you changed something } `
